...
Separate from the ROS 2 driver above, this repo also tracks the files used to
put the UNO Q's own Linux side into USB host mode and to stream, view, record,
and replay point clouds from an attached HPS-3D160-U lidar (a different sensor
from the LD19). That tooling is documented in
**[`docs/unoq-hps3d160.md`](docs/unoq-hps3d160.md)**, covering:
- `usb-role-host.service`, `verify_hps3d160.py`, `backup_unoq_config.sh` —
  USB host-mode setup, link verification, and pre-reflash backup.
- `hps3d160_stream.c` / `hps3d160-stream.service` — the board-side capture
  and TCP streamer, plus its systemd unit.
- `pointcloud_viewer.py` / `pointcloud_replay.py` — the desktop 3D viewer,
  `--record` capture format, and replay.
- Restoring everything after an App Lab OS re-flash.
